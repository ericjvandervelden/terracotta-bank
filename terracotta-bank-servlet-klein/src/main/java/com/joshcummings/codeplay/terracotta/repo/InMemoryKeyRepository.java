...
public class InMemoryKeyRepository implements KeyRepository {
	...
	public void store(String alias, KeyPair pair) {
		...
		// pair.getPrivate() is runtime een RSAPrivateCrtKeyImpl, dus daarom kun je RSAPrivateCrtKeySpec.class geven als 2de arg,
		// dat zal runtime niet fout gaan,
		// RSAPrivateCrtKeySpec heeft alle methods, zoals .getPrimeExponentP()
		// pair.getPrivate() is compile time een PrivateKey, dus heeft alleen .getEncoded(),
		// de factory decodes en geeft een RSAPrivateCrtKeySpec, die .getPrimeP(), ... heeft, 
		//  maar als je ((RSAPrivateCrtKeyImpl)pair.getPrivate()) dan heeft die ook .getPrimeP(), ... TODO
		// RSAPrivateCrtKeySpec is een java pkg, RSAPrivateCrtKeyImpl een sun pkg,
	...
	public <T extends Key> T load(String alias) {
		...
		if (spec instanceof SecretKeySpec) {
			return (T) spec;	// de storage representation is the same as the in-memory representation TODO
		} else {
			return (T) KeyFactory.getInstance("RSA").generatePrivate(spec);
			// sun.security.rsa.RSAPrivateCrtKeyImpl, want spec is een RSAPrivateCrtKeySpec,
		}
	}
}
